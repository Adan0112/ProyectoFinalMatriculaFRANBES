<?php require_once 'db_con.php'; 
	...
	if (isset($_POST['register'])) {
		...
		$input_error = array();
		if (empty($name)) {
			$input_error['nombre'] = "Es necesario diligencia el campo de Nombre";
		}
		if (empty($email)) {
			$input_error['email'] = "Es necesario diligencia el campo de Correo";
		}
		if (empty($username)) {
			$input_error['nombre_usuario'] = "Debes diligenciar el campo de usuario";
		}
		if (empty($password)) {
		...
		if (count($input_error)==0) {
			...
			if (mysqli_num_rows($check_email)==0) {
				$check_username= mysqli_query($db_con,"SELECT * FROM `usuario` WHERE `nombre_usuario`='$username';");
				if (mysqli_num_rows($check_username)==0) {
					if (strlen($username)>7) {
						if (strlen($password)>7) {
								$password = sha1(md5($password));
							$query = "INSERT INTO `usuario`(`nombre`, `email`, `nombre_usuario`, `password`, `photo`, `estado_usuario`) VALUES ('$name', '$email', '$username', '$password','$photo_name','inactivo');";
...
<html lang="es">
  ...
  <h1 class="text-center">Academia de Natacion "Franbes"</h1>
            ...
            <div class="col-md-8 offset-md-2">
             	<form method="POST" enctype="multipart/form-data">
				  <div class="form-group row">
				    <div class="col-sm-6">
				      <input type="text" class="form-control" value="<?= isset($name)? $name:'' ?>" name="nombre" placeholder="Nombre" id="inputEmail3"><?= isset($input_error['nombre'])? '<label for="inputEmail3" class="error">'.$input_error['nombre'].'</label>':'';  ?>
				    </div>
				    <div class="col-sm-6">
				  ...
				  <div class="form-group row">
				  	<div class="col-sm-4">
				      <input type="text" name="nombre_usuario" value="<?= isset($username)? $username:'' ?>" class="form-control" id="inputPassword3" placeholder="Usuario"><?= isset($input_error['nombre_usuario'])? '<label class="error">'.$input_error['nombre_usuario'].'</label>':'';  ?><?= isset($username_error)? '<label class="error">'.$username_error.'</label>':'';  ?><?= isset($usernamelan)? '<label class="error">'.$usernamelan.'</label>':'';  ?>
				    </div>
				    <div class="col-sm-4">
	...
	<br><br><br>
	<p class="text-center">Si tienes una cuenta de acceso administrativo, puedes <a href="login.php">Ingresar Aquí</a></p>
			
    </div >
		<br><br>		  
    	<h2 class="text-center">Especilidad: <a href="#">Analisis de Sistemas</a></h2>
