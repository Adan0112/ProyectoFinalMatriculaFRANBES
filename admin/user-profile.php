...
<?php 
  $query = mysqli_query($db_con, "SELECT * FROM `usuario` WHERE `nombre_usuario` ='$user';");
...
<div class="row">
  <div class="col-sm-6">
    <table class="table table-bordered">
      ...
      <tr>
        <td>Estado</td>
        <td><?php echo ucwords($row['estado_usuario']); ?></td>
      </tr>
      <tr>
  ...
  <div class="col-sm-6">
    ...
    <?php 
        if (isset($_POST['upphoto'])) {
          ...
          $upphoto = $user.date('s-m-y-m-Y').$_FILES['userphoto']['nombre'];
          if (mysqli_query($db_con, "UPDATE `usuario` SET `photo` = '$upphoto' WHERE `usuario`.`nombre_usuario` = '$user';")) {
            move_uploaded_file($photofile, 'images/'.$upphoto);
          }else{
            echo "Imagen de perfil no cargada";
          }
        }
     ?><br>
